...
# SET 1: Personal Datasets
PERSONAL_FILE_PATHS = [
    'Data_Preprocessing/Personal_Data/MariaButaslac.csv', 
    'Data_Preprocessing/Personal_Data/HyacinthToribio.csv', 
    'Data_Preprocessing/Personal_Data/SofiaBorcelo.csv'
]
# SET 2: Public Datasets
PUBLIC_FILE_PATHS = [
    'Data_Preprocessing/Public_Data/public1.csv', 
    'Data_Preprocessing/Public_Data/public2.csv', 
    'Data_Preprocessing/Public_Data/public3.csv', 
    'Data_Preprocessing/Public_Data/public4.csv'
]

# filter out NaN errors clutter: unimportant warnings
...
# MAIN PREPROCESSING FUNCTION : Scaling, Encoding, Imputation, Cleaning

def preprocess_and_save(file_paths, output_filename, artifact_prefix=""):
    ...
    print(f"\n! Merged Datasets ({output_filename}): SHAPE {df_master.shape}")
    ...
    # Save artifacts for the FIRST (personal) dataset only
    if 'personal' in output_filename.lower():
        joblib.dump(le, f"{artifact_prefix}Security_Artifacts/DP_activity_label_encoder.pkl")
        joblib.dump(preprocessor, f"{artifact_prefix}Security_Artifacts/DP_main_preprocessor.pkl") 
        print(f"✅ Artifacts saved: {artifact_prefix}DP_activity_label_encoder.pkl and {artifact_prefix}DP_main_preprocessor.pkl")

    print(f"\nFINAL SHAPE: {X_processed_df.shape[1]} features")
    print(f"\tFinal Dataset saved at: {output_filename}\n")
...
preprocess_and_save(
    PERSONAL_FILE_PATHS,
    "Data_Preprocessing/MERGEDpersonaldataset.csv"
)
...
preprocess_and_save(
    PUBLIC_FILE_PATHS,
    "Data_Preprocessing/MERGEDpublicdataset.csv",
)